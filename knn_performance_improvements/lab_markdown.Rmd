...
for (k in 1:160) {
  ...
  chisq_result <- chisq.test(wbcd_test_labels, wbcd_test_pred)
  new_row <- data.frame(K=k, FalsePositives=results[[1]], FalseNegatives=results[[2]], PVal=chisq_result$p.value)
  false_counts <- rbind(false_counts, new_row)
}
```

To get a visual representation of model performance, let's plot the number of false positives and false negatives for each k.
